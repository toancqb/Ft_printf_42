...
/*   Updated: 2019/02/26 14:58:56 by qtran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_float_width_precision(double f, int p, char **str)
{
	intmax_t	d;
	int			len;
	int			i;
...
void	flag_f(t_env *vn, double f, char **str)
{
	...
	len += (vn->precision) ? 1 : 0;
	if (vn->width > len)
	{
		len = vn->width - len;
		if (vn->minus == 1)
			ft_minus_f(str, len, c);
		else
		{
			if (vn->plus || vn->space)
				len--;
...
void	print_f(t_env *vn, va_list args, int *i)
{
	...
	if (!vn->point && !vn->precision)
		vn->precision = 6;
	get_float_width_precision(f, vn->precision, &buffer);
	if (f > 0 && vn->plus)
		pad_right(&buffer, 1, '+');
